...
export default function Dashboard({ data }: { data: Dataset }) {
  ...
  const ranking = useMemo(
    () => data.venues.slice(0, topN).map((v) => ({
      name: v.short, count: v.count, level: v.level, zone: v.zone, type: v.type === 'journal' ? '期刊' : '会议',
    })),
    [data, topN],
  )
            ...
            <div className="flex gap-2">
              {[10, 15, 27].map((n) => (
                <button
                  key={n}
                  onClick={() => setTopN(n)}
                  className={`rounded-md px-2.5 py-1 text-xs transition ${topN === n ? 'bg-orange-500/20 text-orange-300' : 'text-slate-500 hover:text-slate-300'}`}
                >
                  Top {n === 27 ? '全部' : n}
                </button>
              ))}
            ...
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={ranking} layout="vertical" margin={{ left: 8, right: 40 }}>
                ...
                <Tooltip
                  ...
                  formatter={(v: number, _n, item) => [`${v} 篇`, `${item.payload.type} · ${item.payload.level === 'N' ? '非CCF' : `CCF-${item.payload.level}`}${item.payload.type === '期刊' ? ` · 中科院${item.payload.zone}` : ''}`]}
                />
