...
function PaperCard({ p }: { p: Paper }) {
  ...
  return (
    <div className="rounded-lg border border-slate-800 bg-slate-950/60 p-4 transition hover:border-slate-700">
      <div className="flex items-start justify-between gap-3">
        <div className="min-w-0">
          <a
            ...
            target="_blank"
            rel="noreferrer"
            className="font-medium leading-snug text-slate-100 hover:text-orange-300"
          >
            {p.title}
          </a>
        ...
        <div className="flex shrink-0 flex-col items-end gap-1">
          <span className="rounded bg-orange-500/15 px-2 py-0.5 text-xs font-semibold text-orange-300">{p.year}</span>
      ...
      {p.abstract ? (
        <div className="mt-2">
          <button onClick={() => setOpen(!open)} className="text-xs text-orange-400 hover:text-orange-300">
            {open ? '收起摘要 ▲' : '展开摘要 ▼'}
          </button>
      ...
      >
        ...
        <div className="min-w-0 flex-1">
          <div className="flex items-center gap-2">
            ...
            {v.type === 'journal' && (
              <span className="rounded border border-orange-500/40 bg-orange-500/10 px-1.5 py-0.5 text-[10px] text-orange-300">
                中科院 {v.zone}
              </span>
            )}
          </div>
          ...
          <ResponsiveContainer width="100%" height="100%">
            <BarChart data={trend}>
              <Bar isAnimationActive={false} dataKey="n" radius={[2, 2, 0, 0]}>
                {trend.map((t, i) => (
                  <Cell key={t.year} fill={['#1e293b', '#334155', '#475569', '#38bdf8', '#fb7185'][i]} />
                ))}
              </Bar>
            </BarChart>
          </ResponsiveContainer>
        </div>
        <span className="rounded-full bg-orange-500/15 px-3 py-1 text-sm font-bold text-orange-300">{v.count}</span>
        <svg
      ...
      {expanded && (
        <div className="border-t border-slate-800 px-5 py-4">
          ...
          <div className="flex flex-col gap-3 md:flex-row md:items-center">
            <div className="relative flex-1">
              ...
              <input
                ...
                className="w-full rounded-lg border border-slate-700 bg-slate-950 py-2 pl-9 pr-3 text-sm text-slate-200 placeholder-slate-600 outline-none focus:border-orange-500"
              />
            </div>
            <select
              value={sort}
              onChange={(e) => setSort(e.target.value as SortKey)}
              className="rounded-lg border border-slate-700 bg-slate-950 px-3 py-2 text-sm text-slate-300 outline-none focus:border-orange-500"
            >
          ...
          <div className="mt-3 flex flex-wrap items-center gap-2">
            <button
              onClick={() => setTag(null)}
              className={`rounded-full px-2.5 py-1 text-[11px] transition ${!tag ? 'bg-orange-500/20 text-orange-300' : 'bg-slate-800 text-slate-400 hover:text-slate-200'}`}
            >
              全部方向
            </button>
            {venueTags.map((t) => (
              <button
                ...
                className={`rounded-full px-2.5 py-1 text-[11px] transition ${tag === t ? 'bg-orange-500/20 text-orange-300' : 'bg-slate-800 text-slate-400 hover:text-slate-200'}`}
              >
                {t}
              </button>
            ))}
          ...
          <div className="mt-3 text-xs text-slate-500">
            命中 <span className="font-semibold text-orange-300">{papers.length}</span> / {v.count} 篇
          </div>
...
export default function VenueExplorer({ venues }: { venues: Venue[] }) {
  ...
  const [level, setLevel] = useState<'all' | 'A' | 'B' | 'C' | 'N'>('all')
  ...
  return (
    <section className="mx-auto max-w-7xl px-6 pb-20">
        ...
        <div className="flex flex-col gap-3 md:flex-row md:items-center">
          <div className="relative flex-1">
            ...
            <input
              ...
              className="w-full rounded-lg border border-slate-700 bg-slate-900 py-2 pl-9 pr-3 text-sm text-slate-200 placeholder-slate-600 outline-none focus:border-orange-500"
            />
          </div>
          <div className="flex gap-2">
            {([['all', '全部'], ['journal', '期刊'], ['conference', '会议']] as const).map(([k, label]) => (
              <button
                ...
                className={`rounded-lg px-3 py-2 text-sm transition ${type === k ? 'bg-orange-500/20 text-orange-300' : 'text-slate-500 hover:text-slate-300'}`}
              >
                {label}
              </button>
            ))}
            <span className="my-2 w-px bg-slate-800" />
            {(['all', 'A', 'B', 'C', 'N'] as const).map((k) => (
              <button
                key={k}
                onClick={() => { setLevel(k); setPage(1) }}
                className={`rounded-lg px-3 py-2 text-sm transition ${level === k ? 'bg-orange-500/20 text-orange-300' : 'text-slate-500 hover:text-slate-300'}`}
              >
                {k === 'all' ? '全部等级' : k === 'N' ? '非CCF' : `CCF-${k}`}
              </button>
            ))}
          </div>
        </div>
      ...
      <div className="mt-4 text-xs text-slate-500">
        共 <span className="text-orange-300">{filtered.length}</span> 个期刊/会议 · 第 {cur} / {pages} 页
      </div>
        ...
        <div className="mt-6 flex items-center justify-center gap-2">
          ...
          {Array.from({ length: pages }, (_, i) => i + 1).map((n) => (
            <button
              key={n}
              onClick={() => setPage(n)}
              className={`rounded-lg px-3 py-1.5 text-sm transition ${n === cur ? 'bg-orange-500/25 text-orange-300' : 'border border-slate-800 text-slate-400 hover:border-slate-600'}`}
            >
              {n}
            </button>
          ))}
